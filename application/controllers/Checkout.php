...
class Checkout extends MY_Controller
{
    ...
    {
        $kec = file_get_contents(base_url('assets/kec.json'));

        $data_kec  = json_decode($kec, true);

        $data['kec'] = (object)$data_kec['kec'];

        $this->checkout->table = 'cart';

        $data['cart'] = $this->checkout->select(
            [
                'cart.id', 'cart.id_user AS user_id', 'cart.id_product', 'cart.qty', 'cart.subtotal',
        ...
        if (!$data['cart']) {
            $this->session->set_flashdata('warning', 'Tidak ada produk didalam keranjang anda');
